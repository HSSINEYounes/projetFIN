        ...
        if name not in nameList:
            ...
            for nom in names :
                if nom[0].upper() not in ListPresence :
                    s = findByCne(nom[0])
                    f.writelines(f'\n{nom[0].upper()},{s.nom.upper()},{s.prenom},{s.genre},{s.filiere.nomeF},{dtString},{"Absent(e)"},{matiere}')
                    print(ListPresence)
...
def send_email(request):
    ...
    return render(request, 'etudiant_list.html', {'email_form': email_form})